...
from utils.config import LOG_PATH, Config, BASE_PATH

today = datetime.strftime(datetime.now(), '%Y-%m-%d')
class Logger(object):
    def __init__(self, logger_name='supply-chain'):
        self.logger = logging.getLogger(logger_name)
        logging.root.setLevel(logging.NOTSET)
        c = Config().get('log')
        # self.log_file_name = c.get('file_name') if c and c.get('file_name') else 'test.log'  # 日志文件
        self.log_file_name = os.path.join(BASE_PATH, 'log', 'test_{}.log'.format(today))
